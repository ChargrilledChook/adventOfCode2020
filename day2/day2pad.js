...
/* Two main problems. First problem is wrangling the data into a useable state, and
the second is implementing the logic.

For problem one, there are two main subproblems - how to structure or split up the entire set,
and how to structure each individual entry.

*/

let testMapInput = "1-3 a: abcde";
...
function createPasswordArray(input) {
  const convertData = input.split("\n");
  return convertData.map((entry) => converter(entry));
}

//console.log(converter(testMapInput));

/*
For the logic, we need to correctly validate a single password, and then iterate over a data set
and count the amount of valid passwords.
*/
...
let objArray = [testObj, testObj2, testObj3];
